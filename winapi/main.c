	...
	LPARAM lParam)    // second message parameter
{
	...
	// get cursor position 
	case WM_MOUSEMOVE:
		posXmys = cursorPosition.x;
		posYmys = cursorPosition.y;
		InvalidateRect(hWnd, NULL, TRUE);
		break;
...
void paintObject(HWND hWnd, HDC hDC, PAINTSTRUCT ps, int posX, int posY, POINT cursorPosition)
{
	...
	if (farba == 0) SetDCBrushColor(hDC, RGB(255, 0, 0));
	else SetDCBrushColor(hDC, RGB(0, 0, 255));

	SelectObject(hDC, GetStockObject(DC_BRUSH));

	Ellipse(hDC, 350 + posX, 250 + posY, 550 + posX, 350 + posY);
	Rectangle(hDC, 300 + posX, 300 + posY, 600 + posX, 400 + posY);
	SetDCBrushColor(hDC, RGB(128, 255, 0));
	Ellipse(hDC, 300 + posX, 320 + posY, 400 + posX, 420 + posY);
	Ellipse(hDC, 500 + posX, 320 + posY, 600 + posX, 420 + posY);
	MoveToEx(hDC, 450 + posX, 250 + posY, NULL);
	LineTo(hDC, posXmys, posYmys, NULL);
	return;
}
